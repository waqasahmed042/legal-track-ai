...
import { acceptableProps } from '../../utilities/type';
import Loader from '../../../../components/loader/Loader';

const Page: React.FC<acceptableProps> = ({ acceptableStrikedClause, selectedCardData, isLoading }) => {
    const [editingSection, setEditingSection] = useState<'clean' | 'redline' | null>(null);
    const [editedText, setEditedText] = useState(selectedCardData?.data?.acceptable_final_clause?.final_clause || '');
    const [editedRedlineText, setEditedRedlineText] = useState(acceptableStrikedClause || '');
    const [displayText, setDisplayText] = useState(selectedCardData?.data?.acceptable_final_clause?.final_clause || '');
    const [displayRedlineText, setDisplayRedlineText] = useState(acceptableStrikedClause || '');

    useEffect(() => {
        setEditedRedlineText(acceptableStrikedClause || '');
        setDisplayRedlineText(acceptableStrikedClause || '');
    }, [acceptableStrikedClause]);

    useEffect(() => {
        setDisplayText(selectedCardData?.data?.acceptable_final_clause?.final_clause || '');
    }, [selectedCardData]);
    ...
    return (
        <>
            <Box sx={{ width: "92%", margin: '20px 40px', borderRadius: 2 }}>
                ...
                >
                    <Typography variant="h4" fontWeight="bold">{UIText.acceptable.analysis}</Typography>
                    <Typography variant="body1">{UIText.acceptable.description}</Typography>
                ...
                >
                    <Typography variant="h4" sx={{ fontWeight: "bold" }}>{UIText.acceptable.ai_redraft_clean_ersion}</Typography>
                    <Typography variant="body1" sx={{ padding: "12px 0" }}>{UIText.acceptable.description_1}</Typography>
                    {editingSection === 'clean' ? (
                        <TextField
                            fullWidth
                            multiline
                            value={editedText}
                ...
                >
                    <Typography variant="h4" sx={{ fontWeight: "bold" }}>{UIText.acceptable.ai_redraft_redline_version}</Typography>
                    <Typography variant="body1" sx={{ padding: "12px 0" }}>{UIText.acceptable.description_3}</Typography>
                    ...
                    >
                        {editingSection === 'redline' ? (
                            ...
                            <Button
                                ...
                                disabled={isLoading || !acceptableStrikedClause}
                            >
                                {UIText.buttons.edit}
                            </Button>
                        )}
